  ...
  return (
    <aside className="w-64 bg-white shadow-lg flex flex-col h-full">
      ...
      <nav className="flex-1 px-4 py-6 space-y-2">
        ...
        <button onClick={() => navigateTo('settings')} className={currentPage === 'settings' ? activeLinkClasses : linkClasses}>
          <SettingsIcon />
        ...
        {metrics && (
            <div className="space-y-4 mb-4">
                ...
                <div className="flex items-center text-sm font-medium text-gray-600">
                    <SalesIcon />
                    <span>Today's Sales: <span className="font-bold text-gray-900">₹{(metrics.todaySales || 0).toLocaleString('en-IN')}</span></span>
                </div>
                <div className="flex items-center text-sm font-medium text-gray-600">
                    <MonthlySalesIcon />
                    <span>Monthly Sales: <span className="font-bold text-gray-900">₹{(metrics.monthlySales || 0).toLocaleString('en-IN')}</span></span>
                </div>
            </div>
        )}
